...
def main(args):
    ...
    for epoch in range(init_epoch,args.epoch):
        ...
        for i, data in tqdm(enumerate(dataloader, 0),total=len(dataloader),smoothing=0.9):
            points, target = data
            # points.shape = [1000,4096,9]
            # target.shape = (1000, 4096)
            ...
            # print("pred:",pred.shape)  # torch.size = [12, 4096, 13]
            # print("target: ",target.shape)  # torch.size = [12, 4096]

            pred = pred.contiguous().view(-1, num_classes)
            target = target.view(-1, 1)[:, 0]
            loss = F.nll_loss(pred, target)  # The negative log likelihood loss?? softmax + Cross-Entropy cost
            writer.add_scalars('Train_record/Loss per step',{'loss':loss},i)
            if args.model_name == 'pointnet':
        ...
        writer.add_scalars('Train_record/mIoU',{'accuracy':test_metrics['accuracy']},epoch)
        writer.add_scalars('Train_record/mIoU',{'mIoU':mean_iou},epoch)
